...
export function resolveTokenAddress(symbol: string, chainId: number): string {
  ...
  if (!chainTokens) {
    throw new Error(`No token data for chain ${chainId}`);
  }
  ...
  // If input looks like an address, return as-is
  if (symbol.startsWith('0x') && symbol.length === 42) return symbol;

  const available = Object.keys(chainTokens).join(', ');
  throw new Error(
    `Unknown token "${symbol}" on chain ${chainId}. Available: ${available}`
  );
}

export function getTokenDecimals(symbol: string): number {
  const upper = symbol.toUpperCase();
  if (['USDC', 'USDT', 'USDC.E'].includes(upper)) return 6;
  if (['WBTC'].includes(upper)) return 8;
  return 18; // ETH, BNB, MATIC, AVAX, etc.
}

export function parseAmount(amount: string, tokenSymbol: string): string {
